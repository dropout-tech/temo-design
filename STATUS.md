...
**孤兒改動銷案**：先前 STATUS 記錄的兩個「不明歸屬未提交改動」（layout.tsx 字重 900、
category-landing-client 的 & 樣式）正是此需求的半成品——本次確認完整後提交。
& 保留較小字級（設計上的層次），顏色與字重改繼承大標的 font-black 黑色；
layout.tsx 補載 Noto Sans TC 900 字重讓 font-black 為真實字重。
next-env.d.ts 的 dev/build 自動生成差異已 restore 未提交。

**驗證**：依「小視覺微調直接改+push」慣例未跑 Playwright；改動為單純 class 移除＋字重載入。
部署後可直接看 temo-design.vercel.app/services/brand-graphic 確認。

### 本次完成：作品探索分類頁依分類顯示各自作品（已上線已驗，commit 6d2790d＋e117799）

**需求（使用者原話）**：作品探索下方的作品要依照分類分好，之前有出現點到每個分類
都是一樣的作品很困擾。

**根因**：四個 /services/[slug] 落地頁的篩選設定全部寫死 "all"（當年註記「暫時對不到
單一執行項目」的舊坑），且 11 個執行項目全是平面類，沒有公共藝術/產品/工藝可掛。

**做法**：
- migration 0018：category_groups 加 `landing_slug`（執行項目→服務頁歸屬；11 個平面類
  →brand-graphic）＋新增「公共藝術/產品設計/工藝設計」三個執行項目各自歸屬（共 14 項）。
  已套進正式 Supabase（使用者授權），可安全重跑。
- 前台：落地頁只顯示歸屬自己的執行項目作品（PortfolioGrid 新 prop allowedGroups，
  疊在使用者篩選之上）；brand-graphic 頁篩選下拉只列自家 11 類；**migration 未套用時
  全站行為照舊**（getCategoryGroups 兩段容錯查詢，已實測 12/12）。
- 後台「作品分類」：每列新增「服務頁」歸屬下拉（選了立即存檔）；新分類自動出現在
  作品表單的執行項目下拉。
- 順手修（e117799）：/explore 翻牌面板「查看案例」原連 /portfolio?group=branding 等
  三個不存在的舊值（等同顯示全部），改連對應 /services/[slug]。

**驗證**：tsc 0 錯（僅 .next 既知假錯）、build ✓ 39/39；確定性 Playwright
（scratchpad/verify-landing-split.js）——套用前防禦路徑 12/12、套用後本地 15/15、
**正式站 15/15**（brand-graphic 顯示全部 8 筆、公共藝術/產品/工藝三頁 0 筆＋空狀態訊息、
/portfolio 下拉 15 選項、手機 375 無溢出）；brand-graphic 下拉實測 12 選項、無混入他頁分類。

**下一步（使用者）**：
- 現有 8 筆作品都是平面類，公共藝術/產品/工藝三頁目前是「作品即將上線」空狀態。
  登入 /studio → 作品 → 執行項目改選「公共藝術/產品設計/工藝設計」（或新增作品時選），
  作品就會出現在對應分類頁。
- 後台「作品分類」的「服務頁」下拉需登入實測（照既有存檔模式寫，未經登入實測）。

**地雷**：
- 執行項目設「不掛服務頁」的分類：作品只出現在 /portfolio 總覽，不會出現在任何分類頁。
- 前台 ISR 快取 60 秒且每頁獨立、被請求後才背景更新——後台改完資料，前台要等
  60-120 秒＋重新整理才看得到。
- ~~孤兒改動待查明歸屬~~ → 已銷案：即「& 符號改黑色粗體」需求的半成品，
  2026-07-24 已提交上線（a7939b8，見最上方紀錄）。

---

### 同日稍早：作品內容區塊「文字」升級富文本編輯器（已 push ba7c4cc，Vercel 自動部署）
...
**地雷**：~~工作區兩個不明歸屬未提交檔~~ → 已銷案：即「& 符號改黑色粗體」需求，
2026-07-24 已提交上線（a7939b8）。
