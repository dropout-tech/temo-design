...
import { getWorkPublicPath, SITE_ORIGIN } from "@/lib/legacy-work-paths"
...
export async function generateMetadata(
  ...
  if (!project) return { title: "找不到作品", robots: { index: false } }
  const url = `${SITE_ORIGIN}${getWorkPublicPath(project.slug)}`
  return {
    title: project.title,
    description: project.description || undefined,
    alternates: { canonical: url },
    openGraph: {
      type: "article", locale: "zh_TW", siteName: "TEMO DESIGN",
      title: project.title, description: project.description || undefined, url,
      images: project.cover ? [{ url: project.cover, alt: project.title }] : [],
    },
    twitter: {
      card: "summary_large_image", title: project.title,
      description: project.description || undefined,
...
export default async function PortfolioDetailPage(
  ...
  const publicPath = getWorkPublicPath(project.slug)
  if (!props.preserveLegacyPath && (canonicalSlug || !publicPath.startsWith("/portfolio/"))) {
