      ...
      <div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            className={cn(
              "px-4 py-2 text-[11px] tracking-[0.2em] uppercase transition-all duration-200 rounded-sm border",
              filters.group === "all"
  ...
  return (
    <div className="relative">
      ...
      <input
        ...
        aria-label="搜尋作品"
        className="w-full bg-[#161412] border border-white/10 hover:border-white/20 focus:border-temo-gold text-white placeholder:text-white/30 text-sm pl-11 pr-24 py-3.5 rounded-sm focus:outline-none transition-colors"
      />
  ...
  return (
    <label className="flex flex-col gap-1.5">
      ...
      <select
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="bg-[#161412] border border-white/10 hover:border-white/25 text-white/80 text-xs px-3 py-2 rounded-sm focus:outline-none focus:border-temo-gold transition-colors min-w-[140px] cursor-pointer"
      >
        {options.map((o) => (
...
function ActivePill({ label, onRemove }: { label: string; onRemove: () => void }) {
  return (
    <button
      onClick={onRemove}
      className="group inline-flex items-center gap-1.5 px-2.5 py-1 bg-temo-gold/10 border border-temo-gold/30 text-temo-gold text-[11px] tracking-wider rounded-full hover:bg-temo-gold/20 transition-all"
    >
      {label}
  ...
  return (
    <section className="relative pt-24 md:pt-32 pb-12 md:pb-16 overflow-hidden bg-[#0c0b09]">
      ...
      >
        ...
        <div className="flex gap-10 mt-10 mb-12 md:mb-14">
          {[
            { num: "200+", label: "完成案例" },
            { num: "8", label: "服務類別" },
            { num: "100%", label: "客戶回購率" },
