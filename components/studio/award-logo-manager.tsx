...
import { saveAwardLogo, deleteAwardLogo, reorderAwardLogos } from "@/app/studio/(app)/awards/actions"
...
export function AwardLogoManager({ initial }: { initial: Row[] }) {
  ...
  function commitOrder(next: Row[]) {
    ...
    setOrderError("")
    startOrder(async () => {
      const res = await reorderAwardLogos(ids)
  ...
  return (
    <div className="px-6 md:px-10 py-10 md:py-14 max-w-5xl">
      <div className="flex items-end justify-between mb-8 gap-4">
        <div>
          <p className="text-[10px] tracking-[0.5em] text-temo-gold uppercase mb-2">Awards</p>
          <h1 className="text-3xl md:text-4xl font-bold text-temo-white">得獎紀錄</h1>
          <p className="text-temo-warm-gray/60 text-sm mt-1">
            共 {rows.length} 個 · 顯示在「關於我們」的 Our Honor 牆，拖曳 <span className="text-temo-warm-gray/80">⠿ 把手</span> 排序
            {orderPending && (
...
function Card({
  ...
  function del() {
    if (row.id && !confirm("確定刪除這個得獎紀錄？")) return
    startTransition(async () => {
      if (row.id) {
        const res = await deleteAwardLogo(row.id)
        if (res.error) {
          setError(res.error)
          return
        }
    ...
    <div className="rounded-lg border border-white/10 bg-white/[0.02] p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-medium text-temo-white truncate">{row.name || "未命名獎項"}</p>
